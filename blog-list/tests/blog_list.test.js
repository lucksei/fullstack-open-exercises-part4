...
test('verify that deleting a note results in code 204 and the note no longer exists', async () => {
  let response = await api
    .get('/api/blogs')
    .expect(200)

  const noteToDelete = _.find(response.body, blog => blog.title === "Go To Statement Considered Harmful")
  await api
  ...
  response = await api
    .get('/api/blogs')
    .expect(200)
  const noteDeleted = _.find(response.body, blog => blog.title === "Go To Statement Considered Harmful")
  assert(!noteDeleted)
})
